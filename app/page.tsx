...
const Hero = () => {
  ...
  return (
    <section ref={containerRef} className="h-screen flex flex-col justify-center items-center overflow-hidden px-4 relative">
      ...
      <div className="absolute bottom-1/4 right-10 md:right-20 font-mono text-xs md:text-sm  tracking-widest opacity-40 hidden md:block text-right">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-px bg-black">
          {experiences.map((exp, i) => (
            <div key={i} className="bg-inherit border border-current p-12 hover:bg hover:text-[--bg-color] transition-colors group">
...
const Projects = () => {
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto">
        <div className="text-2xl font-mono mb-20 uppercase tracking-widest italic">Case Study</div>
        ...
        <div className="space-y-0 border-t border-current">
          {projects.map((project, idx) => (
            <motion.div 
              ...
              className="group relative border-b border-current py-12 flex flex-col md:flex-row md:items-center justify-between transition-colors hover:bg-black hover:text-white px-4"
            >
              ...
              <div className="mt-6 md:mt-0 flex flex-col items-start md:items-end gap-4 z-10">
                <div>
                  <span className="font-mono text-sm uppercase opacity-50 group-hover:opacity-100 tracking-tighter block mb-2">{project.tags}</span>
                  <p className="max-w-sm opacity-0 group-hover:opacity-100 transition-opacity hidden md:block">
                    {project.desc}
                  </p>
                </div>
                <div className="flex gap-3 items-center">
                  <a 
                    ...
                    onClick={(e) => {
                      if (!project.caseStudyLink.startsWith('http')) {
                        e.preventDefault();
                        window.location.href = project.caseStudyLink;
                      }
                    }}
                    className="px-4 py-2 border border-current font-mono text-xs uppercase tracking-wider hover:bg-current hover:text-white transition-colors"
                  >
                    Case Study
                  </a>
                  <a 
  ...
  return (
    ...
    >
      ...
      <nav className="fixed top-0 left-0 w-full p-6 md:p-12 flex justify-between items-center z-[100] font-mono text-xs md:text-sm mix-blend-difference pointer-events-none">
        <div className="bg-white dark:bg-neutral-900 text-black dark:text-white px-2 py-1 pointer-events-auto cursor-pointer font-black uppercase tracking-tighter">AG [2026]</div>
        <div className="text-white dark:text-white pointer-events-auto hidden md:block">
